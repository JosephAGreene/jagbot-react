...
function RoleMenu(props) {
  ...
  const [serverRoles, setServerRoles] = React.useState([]);
  ...
  const handleServerRoleSync = async () => {
    ...
    const res = await BotService.getServerRolesNew(payload);
  ...
  const menuItems = () => {
    ...
    return (
      serverRoles.map((server, index) => {
        return (
          <section>
            <MenuItem className={classes.itemHeading} disabled key={`${server.serverId}-${index}`}>
